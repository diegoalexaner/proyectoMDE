		...
		for (RelacionAbstracta relacion : claseAbstracta.getListaRelaciones()) {
			if(relacion.getMultiplicidadB().equals("*")) {
				...
				if(!relacion.getMultiplicidadB().equals(null) ) {
					if(!relacion.getRolA().contains("owner")) {
						cadena.append("	"+relacion.getRolA()+ " = "+ relacion.getTarget().getNombre() + "()\n\n");
					}
				}
			}
		}
	
		for (ContainmentAbstracta relacionC : claseAbstracta.getListaRelacionesContaiment()) {
			if(relacionC.getMultiplicidadA().equals("*")) {
				cadena.append("	"+relacionC.getRolA()+ " " +"= []\n\n");
			}
