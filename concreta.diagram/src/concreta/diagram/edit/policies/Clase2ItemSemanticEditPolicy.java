...
import concreta.diagram.edit.commands.HerenciaCreateCommand;
import concreta.diagram.edit.commands.HerenciaReorientCommand;
import concreta.diagram.edit.commands.RelacionCreateCommand;
import concreta.diagram.edit.commands.RelacionReorientCommand;
 ...
 * @generated
 */
public class Clase2ItemSemanticEditPolicy extends ConcretaBaseItemSemanticEditPolicy {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (ConcretaVisualIDRegistry.getVisualID(incomingLink) == RelacionEditPart.VISUAL_ID) {
			...
			if (ConcretaVisualIDRegistry.getVisualID(incomingLink) == HerenciaEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
				continue;
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (ConcretaVisualIDRegistry.getVisualID(outgoingLink) == RelacionEditPart.VISUAL_ID) {
			...
			if (ConcretaVisualIDRegistry.getVisualID(outgoingLink) == HerenciaEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(outgoingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
				continue;
	...
	*/
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (ConcretaElementTypes.Relacion_4001 == req.getElementType()) {
			return getGEFWrapper(new RelacionCreateCommand(req, req.getSource(), req.getTarget()));
		}
		if (ConcretaElementTypes.Containment_4002 == req.getElementType()) {
			return getGEFWrapper(new ContainmentCreateCommand(req, req.getSource(), req.getTarget()));
		}
		if (ConcretaElementTypes.Herencia_4003 == req.getElementType()) {
			return getGEFWrapper(new HerenciaCreateCommand(req, req.getSource(), req.getTarget()));
		}
		return null;
	}
